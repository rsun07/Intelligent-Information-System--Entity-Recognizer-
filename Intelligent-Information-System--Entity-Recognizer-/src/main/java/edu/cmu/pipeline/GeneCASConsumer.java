 ...
 * @author Ryan Sun
 * 
 *         The CASConsumer receives each CAS after it has been analyzed by the Analysis Engine. The
 *         CASConsumer will do filter, which ignore the less important GeneName. After that, it will
 *         write the data in required formation to the output document.
 */
public class GeneCASConsumer extends CasConsumer_ImplBase {
   ...
   * Consumer will implement this method to obtain parameter values and perform various
   * initialization steps. It will also initialize the evaluator for evaluating the performance of
   * the CPE.
   * 
  ...
  public void initialize() throws ResourceInitializationException {
    ...
    initOutput();
    /*
     * try { initEvaluator(); } catch (IOException e) { e.printStackTrace(); }
     */
  ...
  /*
   * private void initEvaluator() throws IOException{ String path =
   * "./src/main/resources/data/GENE.eval"; evaluator = new Evaluator(path); }
   */

  /**
   * The processCas function will process the data from the aggregated annotator, Filter the gene
   * that are not suitable, write the correct answer to the output document.
   */
  ...
  /**
   * the destroy() function will close the fileWriter It will also call the method in Eluator class
   * to print the final report of precision, recall and F-score
   */
  public void destroy() {
    if (fileWriter != null) {
